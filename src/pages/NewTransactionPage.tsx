...
import { useCategories, useAddTransaction, useCurrentLocation, useCurrentEmployee, useResidents, useMonthlyClose, useResidentBalances, useAddAuditLog } from '@/hooks/useAppData'
...
export function NewTransactionPage() {
  ...
  // Resident is required for cash_advance
  ...
  const isOverLimit = (() => {
    ...
    return (currentBalance + amountNum) > selectedResident.advance_limit
  })()
  ...
  const isDateValid = (() => {
    ...
    return date <= todayStr && date >= ninetyDaysAgoStr
  })()
  ...
  const handleOcr = async () => {
    ...
    try {
      ...
      // Show suggestions for other fields
      if (result.date || result.description) {
        ...
        {isClosedMonth && (
          <div className="flex items-center gap-1.5 rounded-md bg-warning/10 border border-warning/30 p-3">
            ...
            <div>
              ...
              <p className="text-xs text-warning/80">登録には修正理由の入力が必要です。修正は監査ログに記録されます。</p>
            </div>
          </div>
        )}
          ...
          <div className="flex items-center gap-1.5 rounded-md bg-expense/10 border border-expense/30 p-3">
            ...
            <div>
              ...
              <p className="text-xs text-expense/80">
                上限額: {formatCurrency(selectedResident?.advance_limit ?? 0)} / 現在残高: {formatCurrency(residentBalance?.total_balance ?? 0)} + 今回: {formatCurrency(amountNum)}
              </p>
              {canApproveOverLimit ? (
        ...
        {isClosedMonth && (
          <div>
            ...
            <textarea
              ...
              rows={2}
              placeholder="SV締め済みの月に登録する理由を入力してください"
